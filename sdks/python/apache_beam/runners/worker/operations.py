...
class Operation(object):
  ...
  def progress_metrics(self):
    # TODO progress metrics
    logging.info('ajamato Operation progress_metrics')
  ...
  def element_count_metrics(self, transform_id):
    if len(self.receivers) == 1:
      ...
      # fix its name later, which we do.
      # TODO(robertwb): Plumb the actual name here.
      return [ptransform_int64_metric(
...
class DoOperation(Operation):
  ...
  def progress_metrics(self):
    logging.info('ajamato DoOperation progress_metrics')
    ...
    if self.tagged_receivers:
      ...
      for tag, receiver in self.tagged_receivers.items():
        logging.info('ajamato DoOperation progress_metrics val %s' %
  ...
  def monitoring_infos(self, transform_id):
    logging.info('ajamato DoOperation monitoring_infos')
    infos = super(DoOperation, self).monitoring_infos(transform_id)
    if self.tagged_receivers:
      for tag, receiver in self.tagged_receivers.items():
        logging.info('ajamato DoOperation monitoring_infos val %s' %
            receiver.opcounter.element_counter.value())
        infos.append(ptransform_int64_metric(
            'beam:metric:element_count:v1',
            receiver.opcounter.element_counter.value(),
            ptransform=transform_id,
            tag=str(tag)
